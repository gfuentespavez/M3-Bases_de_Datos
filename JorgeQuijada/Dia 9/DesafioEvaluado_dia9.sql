--Creamos la base de datos llamada cine
CREATE DATABASE cine;
--Nos conectamos a la base de datos
\c cine

--Creamos la tabla llamada peliculas
...
--Creamos la tabla llamada reparto
...
--Insertamos los datos de la tabla peliculas

INSERT INTO peliculas(id, Pelicula, Año_estreno, Director) VALUES
...
(18, 'El Señor de los anillos: Las dos torres', 2002, 'Peter Jackson'),
(19, 'Harry Potter y el Prisionero de Azkaban', 2004, 'Alfonso Cuarón'),
...
(52, 'Lo que el viento se llevó', 1939, 'Victor Fleming'),
(53, 'El Laberinto del Fauno', 2006, 'Guillermo del Toro'),
(54, 'Los increíbles', 2004, 'Brad Bird'),
...
(59, 'Indiana Jones y la última cruzada', 1989, 'Steven Spielberg'),
...
(74, 'Tiburón', 1975, 'Steven Spielberg'),
...
(81, 'Alguien voló sobre el nido del cuco', 1975, 'Milos Forman'),
...
(93, 'Sweeney Todd: El barbero diabólico de la calle Fleet', 2007, 'Tim Burton'),
...
(99, 'Moulin Rouge!', 2001, 'Baz Luhrmann'),
(100, 'Psicosis', 1960, 'Alfred Hitchcock');

--Insertamos los datos de la tabla reparto
...
--Obtenemos el ID de la película Titanic.

SELECT MAX(id) AS id_titanic
FROM peliculas
WHERE pelicula = 'Titanic';
--Tabla del Id de la pelicula Titanic.
 id_titanic
------------
          2
(1 fila)

--Listo a todos los actores que aparecen en la película Titanic.

SELECT reparto.actor AS actores_Titanic
FROM peliculas
JOIN reparto ON peliculas.id = reparto.id_pelicula
WHERE peliculas.Pelicula = 'Titanic'
ORDER BY actor;
--Listado a todos los actores de la pelicula Titanic.
  actores_titanic
-------------------
 Bernard Hill
 Bill Paxton
...
--Consulto en cuántas películas del top 100 participo Harrison Ford.

SELECT COUNT(Peliculas) AS participacion 
FROM peliculas 
JOIN reparto ON peliculas.id = reparto.id_pelicula 
WHERE reparto.actor = 'Harrison Ford';
--Tabla de participacion de Harrison Ford
 participacion
---------------
             8
(1 fila)

--Indico las películas estrenadas entre los años 1990 y 1999 ordenadas por título de manera ascendente.

SELECT Pelicula, MIN(año_estreno) AS año_estreno
FROM peliculas
WHERE año_estreno >= 1990 AND año_estreno <= 1999
GROUP BY Pelicula
ORDER BY Pelicula ASC;
--Listado de peliculas entre 1990 y 1999
...
SELECT Pelicula, MIN(LENGTH(Pelicula)) AS longitud_titulo
FROM Peliculas
GROUP BY Pelicula
ORDER BY Pelicula ASC;
--Titulos con su longitud 
...
--Consulto cual es la longitud mas grande entre los titulos de las peliculas.

SELECT LENGTH(Pelicula) AS longitud_mas_grande
FROM Peliculas 
ORDER BY longitud_mas_grande DESC LIMIT 1;
--Tabla con la longitud mas grande
 longitud_mas_grande
---------------------
                  52
(1 fila)

--Me salgo de postgres
\q