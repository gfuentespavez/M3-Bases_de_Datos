--creo una base de datos llamada peliculas
...
--inserto datos en tabla movie
INSERT INTO movies (id, pelicula, año_estreno, director) VALUES
...
ALTER TABLE movies
ALTER COLUMN pelicula TYPE VARCHAR(100);

--Inserto datos en tabla reparto
...
SELECT reparto.actor FROM movies JOIN reparto ON movies.id = reparto.id_pelicula WHERE movies.pelicula = 'Titanic';
...
SELECT COUNT(movies) AS harrison_veces FROM movies JOIN reparto ON movies.id = reparto.id_pelicula 
...
SELECT * FROM movies WHERE año_estreno BETWEEN 1990 AND 1999 ORDER BY pelicula ASC;
...
SELECT Pelicula, LENGTH(Pelicula) AS longitud_titulo FROM movies;
...
SELECT pelicula
FROM movies
WHERE LENGTH(pelicula) = (SELECT MAX(LENGTH(pelicula))
FROM movies
);
                       pelicula
------------------------------------------------------
...
SELECT LENGTH(pelicula) AS mas_grande FROM movies ORDER BY mas_grande DESC LIMIT 1;
 mas_grande
------------
         52
(1 row)
